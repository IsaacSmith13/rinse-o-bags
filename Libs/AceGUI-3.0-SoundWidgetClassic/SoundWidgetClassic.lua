-- SoundWidgetClassic.lua is based on AceGUI-3.0-SharedMediaWidgets\SoundWidget.lua
-- modified to work in Classic (PlaySoundFile replaced by PlaySound)
------------------------------------------------------------------------------------
...
do
	local widgetType = "LSM30_SoundClassic"
	...
	local function PlaySoundClassic(soundKitID, channel)
		if soundKitID and tonumber(soundKitID) then
			PlaySound(soundKitID, "Master")
			--if DLAPI then
				-- DLAPI.DebugLog(widgetType,"PlaySound(%s, \"Master\")", tostring(soundKitID))
			--end
		else
			PlaySoundFile(soundKitID, "Master")
			--if DLAPI then
				-- DLAPI.DebugLog(widgetType,"PlaySoundFile(%s, \"Master\")", tostring(soundKitID))
			--end
		end
	end
	...
	local function ContentSpeakerOnClick(this, button)
		...
		PlaySoundClassic(self.list[sound] ~= sound and self.list[sound] or Media:Fetch('sound',sound), "Master")
	end
